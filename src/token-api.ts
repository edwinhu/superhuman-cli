 ...
 * @param options - Additional options, including optional pre-fetched threadMessages
 * @returns AI response
 */
export async function askAI(
  ...
  query: string,
  options?: AIQueryOptions & { threadMessages?: FullThreadMessage[] }
  ...
  if (threadId) {
    // Reply mode: use pre-fetched thread messages for context
    if (!options?.threadMessages || options.threadMessages.length === 0) {
      throw new Error("Thread messages must be provided for thread-context AI queries");
    }

    const threadMessages = options.threadMessages.map((m) => ({
