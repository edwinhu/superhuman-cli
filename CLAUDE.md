...
**Session-cookie credential refresh (2026-07-16) — the path that works on Chrome-extension deployments (Linux):** `src/session-refresh.ts` (`refreshViaSessionCookies`) mints fresh tokens by replaying the app's own `Credential.refreshSession()`: `GET accounts.superhuman.com/~backend/v3/sessions.getCsrfToken` → `POST …/sessions.getTokens` with `X-CSRF-Token` + body `{emailAddress, googleId}`, both authenticated by the browser's **Superhuman session cookies** (read via CDP `Network.getCookies` on a **page** target). The backend returns `{authData:{idToken, accessToken, expires}}` — provider-agnostic (Google `ya29.*` or MS Graph JWT), no provider OAuth flow, and the cookies long outlive the ~1h tokens so **no interactive re-auth**. **Both deployments are valid refresh sources, everywhere** — every entry point tries Electron first, then session cookies: `refreshTokenViaCDP()` (on-demand, used by `getCachedToken`/`resolveToken`), `refreshAllTokens()` (bulk), `account auth` (`cmdAuth`), and `doctor` (reports healthy if *either* path works, instead of demanding an Electron relaunch that Linux can't do). Electron gains a silent fallback from this too: when its debug port tears down mid-session (the 9252 issue below), session-cookie refresh now recovers without the focus-stealing nav path. **Why it was needed:** the Electron `background_page.html` target doesn't exist under the Chrome extension, so every refresh failed and `getCachedToken()` fell back to a stale token — invisible to `read`/`inbox`/`search` (SQLite) but fatal to `attachment download`, the only read path hitting a live API. **Critical constraint:** the extension's service-worker and offscreen-document targets often **don't answer CDP `Runtime.evaluate` at all** — not even `1+1`/`Runtime.enable`, while page targets on the same port respond fine. An MV3 service worker idle-stops after ~30s and a stopped worker still appears in `CDP.List()` and still accepts a websocket, it just never replies, so calls hang **forever** unless bounded. This is why `superhuman account auth` hung indefinitely (unbounded `extractTokenChrome`) and why `sync --check` silently reports "app not reachable over CDP" and falls back to on-disk enumeration — both observed 2026-07-16, despite the extension SW path being "verified live" in 76ebd73 (it works only while the worker happens to be awake). All SW evaluates are now bounded by `swEvaluateWithTimeout` (5s). **Never build a refresh path that depends on evaluating JS in an extension context** — use the session-cookie path, which needs none. See `docs/investigations/2026-07-16_attachment_download_401.md`.
